...
| 阶段 C | 钉钉免登、行级权限、Nacos listener 热更新、大屏模式 + **授权中心（ops-web 白屏化授权，运维侧第一个功能）** | 设计已定稿（`specs/2026-09-21-bi-dingtalk-auth-design.md`，M0–M4）；骨架 `common/ops_web/` 已起步 | 全员推广 + 权限操作面收口 | 视需求 |
