...
总经理 BI 看板（14 模块、11 页面：L1 首页驾驶舱 + L2 分析页）的**展示层完全自研**，不走第三方 BI 二开。理由（2026-09-12 用户拍板）：自研服务可以走 Nacos 注册中心、与其他 apps 线服务同一套配置治理一起迭代；Metabase 试点（`http://127.0.0.1:3000/dashboard/2`，5 图看板已验证数据通路）仅留作临时参照与自助取数，不作为主路径。

`bi-web` 是**业务线**新增的长驻 web 服务，与 `robot` / `pages-hangzhou` 同级：

- **只读 `mart_ops`**，不碰 `raw_*`、不碰源、不挂任何源凭据、不外呼钉钉/WDT——凭据边界由 compose 合约测试把守（同 robot 的先例）。
- **配置只取 Nacos**：服务注册条目在 `PIPELINES` 组（`bi-web.yaml`），看板定义在新组 `BI`；种子文件版本受控，Nacos 为准、种子兜底（fail-open 与管线注册表一致）。
- **首批范围**：L1 首页驾驶舱的模块 ①③⑫（差距分析中 A 类已就绪集合），跑 2 周再铺 L2。
